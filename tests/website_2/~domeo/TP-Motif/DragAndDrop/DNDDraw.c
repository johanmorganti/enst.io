...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DNDDraw.c,v $ $Revision: 1.2 $ $Date: 92/03/13 15:33:25 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
    ...
    for (i = appInfo->numRects - 1; i >= 0; i--) {
        ...
        rect = appInfo->rectDpyTable[i];
        if (rect->x <= x && rect->x + (Position)rect->width >= x &&
            rect->y <= y && rect->y + (Position)rect->height >= y) {
            return(rect);
        }
...
#endif /* _NO_PROTO */
{
   ...
   */
    if (maxWidth < width || maxHeight < height) {
        ...
        if (width > height) {
            height = (Dimension) (height * maxWidth) / width;
            width = appInfo->maxCursorWidth;
        } else {
            width = (Dimension) (width * maxHeight) / height;
